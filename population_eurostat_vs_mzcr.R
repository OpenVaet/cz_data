...
df_aug <- df_base %>%
  mutate(
    ...
    YearOfBirth_end = suppressWarnings(as.integer(sub(".*-", "", YearOfBirth))),
...
df_known_yob <- df_alive_2024 %>%
  filter(!is.na(YearOfBirth_start), !is.na(YearOfBirth_end))
...
mzcr_age_ranges <- df_known_yob %>%
  transmute(
    min_age = pmax(0L, REF_YEAR - YearOfBirth_end - 1L),  # birthday may be after Jan 1
    max_age = pmax(0L, REF_YEAR - YearOfBirth_start)      # birthday may be Jan 1 or earlier
  ) %>%
  mutate(
