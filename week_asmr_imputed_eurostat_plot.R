...
create_interactive_asmr_plot <- function() {
  ...
  # Read the ASMR data
  asmr_data <- read.csv("outputs/weekly_asmr_deaths_with_imputation_eurostat_pop.csv", stringsAsFactors = FALSE) %>%
    mutate(
...
if (!file.exists("outputs/weekly_asmr_deaths_with_imputation_eurostat_pop.csv")) {
  stop("Error: outputs/weekly_asmr_deaths_with_imputation_eurostat_pop.csv not found. Please run the main analysis first.")
}
