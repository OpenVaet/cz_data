...
open my $out_imput, '>', 'outputs/imputation_layer_1.csv';
...
while (my $row = $mzcr_csv->getline_hr($mzcr_fh)) {
	...
	if ($week_date_of_death) {
		if ($year_of_birth_end) {
			...
			($from_year, $to_year) = from_year_to_year_from_age($age_at_death);
			my $age_group = "$from_year-$to_year";
...
open my $out_deaths, '>', 'outputs/deaths_offset.csv';
